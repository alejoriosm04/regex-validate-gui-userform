...
numbers = ['0','1','2','3','4','5A','6','7','8','9']

months = {"JAN": 1, "FEB": 2, "MAR": 3, "MAY": 4, "APR": 5, "JUN": 6, "JUL": 7, "AUG": 8, "SEPT": 9, "OCT": 10, "NOV": 11, "DEC": 12}
spanish_months = {"ENERO": 1, "FEBRERO": 2, "MARZO": 3, "MAYO": 4, "ABRIL": 5, "JUNIO": 6, "JULIO": 7, "AGOSTO": 8, "SEPTIEMBRE": 9, "OCTUBRE": 10, "NOVIEMBRE": 11, "DICIEMBRE": 12}
...
def birthDateChecker(date):    
    if re.search(pattern1, date) or re.search(pattern2, date) or re.search(pattern3, date) or re.search(pattern4, date):
        return True
    else:
        return False
...
def tillYourBirthday(day, month, year):
    ...
    if (numberOfDays) > 0: # if the difference is negative, your birthday has already passed, so we need to do the count of next years num of day
...
def containsWeirdSymbols(password):
    for char in password:
        if (char not in capital_alph) and (char not in small_alph) and (char not in special_chars_list) and (char not in numbers):
            return True
        else:
            return False
...
def checkNumber(number):
    pattern_num = "(\d\d\d\d\d\d\d\d\d\d)" 
